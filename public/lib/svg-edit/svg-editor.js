...
//					$('.contextMenu').hide();
	...
	$.each(toolnames, function(i,item) {
        //Prev codes below creates errors in latest JQuery and results in all_tools = '#tool_clear#tool_open...'
        //all_tools += '#tool_' + item + (i==toolnames.length-1?',':'');
        //We need '#tool_clear, #tool_open...', so we modify to below (if in error, pls let me know!)
		all_tools += '#tool_' + item + (i==toolnames.length-1?'':', ');
	});
...
//			});
		...
		editor.importImageFromUrl = function(file, imageId, price, imgw, imgh, imgx, imgy, license) {

			var insertNewImage = function(width, height, imgx, imgy) {
				var newImage = svgCanvas.addSvgElementFromJson({
					element: 'image',
					attr: {
						x: imgx,
				...
				svgCanvas.setHref(newImage, file);
				svgCanvas.selectOnly([newImage]);
				//svgCanvas.alignSelectedElements('m', 'page');
				//svgCanvas.alignSelectedElements('c', 'page');
				//updateContextPanel();
			...
			img.onload = function() {
				imgWidth = imgw;
				imgHeight = imgh;
				insertNewImage(imgWidth, imgHeight, imgx, imgy);
			};
		}
