...
//	if (selected === selectedElements[0]) {
		...
		mgr.imagePriceRect.setAttribute('x', nbax + (nbaw)/2 - 15);
		mgr.imagePriceRect.setAttribute('y', nbay - 7);
		mgr.imagePrice.setAttribute('x', nbax + (nbaw)/2);
		mgr.imagePrice.setAttribute('y', nbay + 4);

		// if(this.selectedElement.tagName == 'text'){
		// 	var id = this.selectedElement.id;
		// 	$('#'+id).removeAttr('transform');
		// }

		//console.log(this.selectedElement);

		var mywidth = selected.getBoundingClientRect().width;
		var myheight = selected.getBoundingClientRect().height;
		var myleft = selected.getBoundingClientRect().left;
		var mytop = selected.getBoundingClientRect().top;

		$("#tools_top").css('display', 'block');
		// $("#tools_top").css('left',myleft - 150);
		// $("#tools_top").css('top',mytop - 70);
		$("#tools_top").css('left','-1000px');

		$('#svgImageWidth').val(mywidth);
		$('#svgImageHeight').val(myheight);
		$('#svgImageLeft').val(myleft);
		$('#svgImageTop').val(mytop);

		//console.log(mywidth+" - "+myheight+" - "+myleft+" - "+mytop);
		
		if(selected.tagName != 'text' && selected.tagName != 'image'){
			$("#tools_top").css('display', 'none');
		}
		//$("#tools_top").css('display', 'none');
		if(selected.tagName == 'image'){

			var fot = $(selected).attr('id');
			...
			var exist = $('#svg_exist').attr('value');

			if(exist == 1){
				$('#workarea image').each(function(){
					var sid = $(this).attr('id');
					var pr = $('#'+sid+"_price").attr('value');
					var lc = $('#'+sid+"_license").attr('value');
					$('#'+sid).attr('price',pr);
					$('#'+sid).attr('license',lc);
				});
			}

			if($(selected).attr('price') > 0){
				var price = $(selected).attr('price');
				$('#imagePrice').text(price).show();
				

				_.each(libs.infos[fot].licenses_details, function(item){
					if((selected.getAttribute('width') > item.width || selected.getAttribute('height') > item.height) && (item.width > 0 || item.height > 0)){
						_.each(libs.infos[fot].licenses, function(item1){
							if(selected.getAttribute('license') == item1.name){
								$('#imagePrice').text(item1.price).show();
								$('#'+id).attr('license',item1.name);
							}
						});
					}
				});

				$(selected).attr('price',$('#imagePrice').text());

				$('#imagePriceRect').show();

				$('#image_price').text('Price: '+$('#imagePrice').text());
				//selected.showGrips(false);

				$('#svg_exist').attr('value','0');

			}else{
				if(libs.infos[fot]){
					delete libs.infos[fot];
					console.log(libs.infos);
				}
				$('#image_price').text('Price: Free');
				$('#imagePriceRect').hide();
				$('#imagePrice').hide();
			}
		}else{
...
svgedit.select.SelectorManager = function() {
	...
	//console.log(this.selectorParentGroup);
...
svgedit.select.SelectorManager.prototype.initGroup = function() {
	...
	this.imagePriceRect = svgFactory_.createSVGElement({
		'element': 'rect',
		'attr': {
			'id': 'imagePriceRect',
			'fill': 'white',
	...
	this.selectorGripsGroup.appendChild(this.imagePriceRect);

	this.imagePrice = svgFactory_.createSVGElement({
		'element': 'text',
		'attr': {
			'id': 'imagePrice',
			'text-anchor': 'middle',
	...
	this.selectorGripsGroup.appendChild(this.imagePrice);
