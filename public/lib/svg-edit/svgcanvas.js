				...
				if (selectedElements[0] != null) {
					...
					if (selectedElements[1] == null) {
						...
						if(selected.tagName == "text"){
							...
							selectorManager.requestSelector(selected).showGrips(false);
						}
...
this.setSvgString = function(xmlString) {
	try {
		...
		var bleed = svgedit.utilities.getElem('bleed');

		bleed = svgdoc.createElementNS(NS.SVG, "rect");
		svgedit.utilities.assignAttributes(bleed, {
			'id': 'bleed',
			'x':svgcontent.getBBox().x + 5.5,
			'y':svgcontent.getBBox().y + 5.5,
			'width': svgcontent.getBBox().width - 11,
			'height': svgcontent.getBBox().height - 11,
			'fill':'#000',
			'fill-opacity':'0.0',
			'stroke':'#337AB7',
			'stroke-width':'11.338583',
			'stroke-opacity':'0.5',
			'style':'pointer-events:none'
		});

		var bleed1 = svgedit.utilities.getElem('bleed1');

		bleed1 = svgdoc.createElementNS(NS.SVG, "rect");
		svgedit.utilities.assignAttributes(bleed1, {
			'id': 'bleed1',
			'x':svgcontent.getBBox().x + 20,
			'y':svgcontent.getBBox().y + 20,
			'width': parseInt(svgcontent.getBBox().width) - parseInt(40),
			'height': parseInt(svgcontent.getBBox().height) - parseInt(40),
			'fill':'#000',
			'fill-opacity':'0.0',
			'stroke':'#E71C1C',
			'stroke-dasharray': '5',
		...
		if($('#bleed').length){
			...
			svgcontent.appendChild(bleed);
			svgcontent.appendChild(bleed1);
		...
		//console.log(svgCanvas.getCurrentDrawing().current_layer.children.item(1));
		//svgCanvas.selectorManager.requestSelector(svgCanvas.getCurrentDrawing().current_layer.children.item(6)).showGrips(true);
		//var s = svgCanvas.getCurrentDrawing().current_layer.children.item(5).getAttribute('id');
		for(var a = 0; a < svgCanvas.getCurrentDrawing().current_layer.children.length; a++){
			var tag = svgCanvas.getCurrentDrawing().current_layer.children.item(a).tagName;
			var s = svgCanvas.getCurrentDrawing().current_layer.children.item(a).getAttribute('id');
			if(tag == 'text' || tag == 'image'){
				var txt = $('#'+s).text();
				$('#svglists').append('<button type="button" class="btn btn-default" onclick="svgCanvas.svgeditsvg('+a+')" id="'+a+'">'+txt+'</button><br/>');
			}
...
this.svgeditsvg = function(v){
	// console.log(svgCanvas.getCurrentDrawing().current_layer.children.item(v));
	// clearSelection(true);
	// svgCanvas.selectorManager.requestSelector(svgCanvas.getCurrentDrawing().current_layer.children.item(v)).showGrips(true);
	var cur_elem = svgCanvas.getCurrentDrawing().current_layer.children.item(v);
	selectOnly([cur_elem], true);
	call("selected", selectedElements);
...
this.updateCanvas = function(w, h) {
	...
	$('#bleed').css('pointer-events','none');
	$('#bleed1').css('pointer-events','none');
